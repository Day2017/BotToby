...
helpMessage ="""
═══════════════════
🔰V.1.2 Beta(คำสั่งย่อ)🔰 
═══════════════════
║> /Protect   [คำสั่ง การป้องกัน]
║> /Steal     [คำสั่ง Steal]
║> /Gift      [คำสั่ง Gift]
║> /Translate [คำสั่ง แปลภาษา]
║> /Socmed    [คำสั่ง Socmed]
║> /Bot       [คำสั่ง บอท]
║> /Setting   [คำสั่ง ตั้งค่า]
╠══════════════════
║> ผู้คุมระบบ :
║=> line://ti/p/~j.days
╠══════════════════
...
wait = {
    ...
    'timeline':True,
    'autoAdd':True,
    'message':"""ผู้ควบคุมบอท : line://ti/p/~j.days""",
    "lang":"JP",
    "comment":"""ผู้ควบคุมบอท : line://ti/p/~j.days""",
    "commentOn":True,
    "commentBlack":{},
    "wblack":False,
    "dblack":False,
...
#------------------NOTIFIED_READ_MESSAGE----------------#
        ...
        if op.type == 26:
            ...
            if "@"+cl.getProfile().displayName in msg.text:
                if wait["tag2"] == True:
                    tanya = msg.text.replace("@"+cl.getProfile().displayName,"")
                    jawab = "มีธุระเร่งด่วนมั้ยเอ่ย ทิ้งข้อความไว้นะครับ "+cl.getProfile().displayName+" ช่วงนี้งานเยอะมาก...\n\n#ตอบรับอัตโนมัติ!!!"
                    jawaban = (jawab)
        ...
        if op.type == 17:
           if wait["Wc"] == True:
               ...
               random.choice(KAC).sendText(op.param1, "💞สวัสดีครับ💞ยินดีต้อนรับสู่ : " + str(ginfo.name))
               #random.choice(KAC).sendText(op.param1, "ผู้สร้างกลุ่มนี้ครับ : " + str(ginfo.name) + " :\n" + ginfo.creator.displayName)
               random.choice(KAC).sendText(op.param1, "😊💞คนมาใหม่แนะนำตัวหน่อยจ้า💞😊")
               print "MEMBER HAS JOIN THE GROUP"
        if op.type == 15:
            if wait["Lv"] == True:
                if op.param2 in Bots:
                    return
                ki.sendText(op.param1, "💞ลาก่อนจ้า😊แล้วพบกันใหม่นะ💞")
                print "MemberLeft"
#-----------------------------------------------
        if op.type == 11:
            if op.param3 == '1':
...
#--------------------------------------------------
		...
		for ticket_id in n_links:
			if wait["atjointicket"] == True:
				...
				cl.sendText(msg.to,"💞ขอบคุณที่เชิญ💞เข้าร่วมกลุ่ม %s" % str(group.name))
#--------------------------------------------------
            elif msg.text == "Ginfo":
                if msg.toType == 2:
